...
grep -q "initializeDailyCheckinStorage" "$ROOT/src/features/profiles/infrastructure/weekly-summary-storage.ts" || { echo "FAIL: canonical check-in ownership linkage missing"; exit 1; }
...
if grep -q "Record today as complete" "$ROOT/src/app/index.tsx"; then
	echo "FAIL: parent completion bypass action should not exist"
	exit 1
fi
